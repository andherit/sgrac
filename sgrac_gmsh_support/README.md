...
`sgrac-gmsh-support` creates the large planar triangular support mesh used before rupture-domain masking.
...
```bash
./sgrac-gmsh-support lx=120000 ly=80000 lc=500 > parent.vtk
```

File-output style:

```bash
./sgrac-gmsh-support lx=120000 ly=80000 lc=500 out=parent.vtk
```
...
A small `FIELD FieldData` block stores `lx_m`, `ly_m`, `lc_m`, and `gmsh_algorithm` for diagnostics.
